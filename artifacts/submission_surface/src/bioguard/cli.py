...
def cmd_check(_: argparse.Namespace) -> int:
    ...
    if config.BENCHMARK_MANIFEST_SCHEMA.exists():
        manifest = read_json(config.DEFAULT_MANIFEST)
        seed_request = manifest.get("seed_request", None)
        request_path = Path(seed_request) if seed_request else Path("artifacts/requests/seed_request.json")
        if request_path.exists():
            request_payload = read_json(request_path)
            contract = read_json(config.DEFAULT_CONTRACT)
            try:
                decision = screen(request_payload, contract, include_sequences=False)
                if isinstance(decision, dict) and "status" in decision and decision["status"] in {400, 422}:
                    status["files"][str(request_path)] = {"status": "skip", "reason": "seed request not valid"}
                else:
                    decision_schema = read_json(config.DEFAULT_DECISION_SCHEMA)
                    result = _schema_validate(decision, decision_schema, label="seed_screen_decision")
                    status["files"]["runtime_seed_screen_contract"] = {
                        "status": result["status"],
                        **({"reason": result.get("reason", result.get("label"))} if result["status"] != "ok" else {}),
                    }
            except Exception as exc:
                status["files"]["runtime_seed_screen_contract"] = {"status": "fail", "reason": str(exc)}
        else:
            status["files"]["runtime_seed_screen_contract"] = {"status": "skip", "reason": "seed request missing"}
...
def cmd_evaluate(ns: argparse.Namespace) -> int:
    ...
    if ns.include_ablations:
        for condition in DEFAULT_ABLATION_STACK:
            if condition not in baselines:
                baselines.append(condition)
...
def cmd_init(ns: argparse.Namespace) -> int:
    ...
    regenerate_artifacts = ns.force_seed or not artifact_test.exists() or not artifact_dev.exists() or not artifact_train.exists()
    split_plan = [
        ("train", artifact_train, seed),
        ("dev", artifact_dev, seed + 1),
        ("test", artifact_test, seed + 2),
    ]
    split_payloads: dict[str, list[dict]] = {}

    if regenerate_artifacts:
        for split_name, path, split_seed in split_plan:
            split_payloads[split_name] = build_seed_corpus(seed=split_seed, target_sizes=target_sizes)
            write_synthetic_corpus(path, split_payloads[split_name])
    else:
        for split_name, path, _ in split_plan:
            rows = read_jsonl(path)
            if rows:
                split_payloads[split_name] = rows
    ...
    seed_corpus = split_payloads.get("test") or split_payloads.get("train") or []
    if not seed_corpus:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_eval.add_argument(
        "--include-ablations",
        action="store_true",
        help="Append pre/post-inference-only ablation baselines to the comparison.",
    )
